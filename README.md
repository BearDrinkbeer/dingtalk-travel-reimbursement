...
本仓库实现一个公司内部钉钉 H5 工具：员工免登后创建持久报销草稿，填写报销项目/预算代码，按需申请出差补助，上传票据和行程单，确认识别结果并选择本人已通过的出差审批。员工最终确认后，服务器按锁定快照生成正式 `.xlsx`，将原始附件和 Excel 直传审批附件空间，再一次性创建钉钉 OA。

基础功能和分阶段验收见 [`docs/V1_IMPLEMENTATION_PLAN.md`](docs/V1_IMPLEMENTATION_PLAN.md)；完整的草稿、附件、出差关联和 OA 提交设计见 [`docs/DINGTALK_OA_INTEGRATION_DESIGN.md`](docs/DINGTALK_OA_INTEGRATION_DESIGN.md)。本文是开发时必须遵守的范围与业务契约；文档冲突时，应先更新并确认文档，不在代码中自行扩大需求。
...
- 基于脱敏公司模板生成 Excel：草稿预览在内存中即时下载，正式提交的 Excel 暂存到受控持久卷，上传并从 OA 回读确认后按生命周期清理本地副本。
- 报销草稿、结构化 OCR 候选、原始附件元数据和提交状态持久保存；票据字节保存在受控 staging 卷，不保存 OCR 原文。
...
- 一个文件多张票据、多页票据拆分。
- 创建 OA 后再修改原表单字段或向原附件控件补文件。
- 发票验真、查重、税务查询。
- 预算控制、ERP 对接、财务付款。
- 机票舱位、住宿限额、自驾标准、跨年发票等合规审核。
- 纸质票据粘贴和电子票据打印排版。
- 代替钉钉的后续审批流转、财务审核、付款或打款后留言。
- Redis、消息队列、微服务、Kubernetes、大模型。

## 实现原则

- 从免登、草稿、票据/OCR、Excel 到 OA 提交使用同一份服务端权威数据；正式提交后只读不可变快照。
...
- Pinia 是页面当前编辑状态，`localStorage` 和 `sessionStorage` 不是权威数据源。员工保存后，报销项目、出差、明细和 OCR 去向与草稿 revision 一起写入 SQLite；刷新或换设备后可按企业、员工和部门恢复。退出或 Session 失效会清空前端身份状态，不会绕过服务端草稿归属和过期规则。
...
## Phase 5：持久草稿附件与本地 OCR

当前报销页使用草稿级文件接口，不把浏览器内存或 30 分钟临时文件当作 OA 提交依据：

- `POST /api/reimbursements/drafts/{draftId}/files` 按 `expectedRevision` 上传一份 `EXPENSE_SOURCE` 或 `ATTACHMENT_ONLY`；元数据和 OCR 状态写入 SQLite，文件字节写入受控持久 staging 卷。页面刷新、进程重启或换设备后可在身份与草稿归属校验通过后恢复。
- `POST /api/reimbursements/drafts/{draftId}/files/{fileId}/ocr` 持久结构化 OCR 候选和状态。OCR 费用行按来源文件 ID 关联；从明细中移除该行会保留原始附件并记录“不计入明细”，不会把文件隐式删除。删除文件必须调用草稿文件 `DELETE` 接口。
- 缺少 `ocrDispositionVersion` 的历史草稿按版本 `0` 读取。只有候选与费用行六个计算字段完全相同且两侧唯一时才自动补 `sourceFileId`；其余终态票据必须由员工明确选择“添加到费用明细”或“忽略此票据”，未全部决定时前后端都禁止保存、复核和正式提交。
- 普通草稿文件保留到草稿 `expiresAt`。正式提交后，原件和最终 Excel 只在 OA 回读确认已关联、孤儿文件已明确清理，或从未触及远端且安全过期时删除本地副本。`MANUAL_REVIEW`、commit 结果不确定或 OA 创建结果不确定时禁止按 TTL 删除。详见 [OA 集成设计的文件生命周期](docs/DINGTALK_OA_INTEGRATION_DESIGN.md#13-文件保留和清理规则)。

### 临时解析接口和资源防护

`/api/files/*` 是受 Session 约束的短期解析边界；它的文件不得用于已锁定 OA 快照。它与上述草稿持久文件共用同样的类型、大小、内容和 OCR 资源防护，但按独立的短期规则清理。
...
前端只提供一个“费用明细”区域，右上角统一放置“手动添加”和“选择票据”。用户一次选择多个独立票据后，上传中/识别中的文件先显示在同一区域，OCR 完成或失败后直接成为带来源与警告标记的可编辑费用行。前端从后端公开配置读取技术保护上限，默认可处理 200 条费用明细；Excel 不再受模板行数限制。删除 OCR 费用行只改变它是否计入明细，原始附件仍随草稿保留；删除附件是独立的明确操作。页面不展示 OCR 原文，不把浏览器存储作为草稿权威数据，也不提供云 OCR 或付费 API 配置。
...
- SQLite 持久卷保存配置、Session、报销草稿、附件元数据、模板目录、不可变提交快照、任务状态和远端操作检查点。原始附件和待提交 Excel 保存在独立 `reimbursement_staging` 持久卷。两个持久卷必须做一致性备份和恢复；只恢复一份会造成数据库哈希与文件不匹配，系统应停止提交而不是继续使用错误文件。短期 `sessions` 不作为业务恢复来源。
