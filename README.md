...
- 出差补助为可选项；申请时填写类型、开始/结束日期和上午/下午，由后端重算。员工只选“境内市外项目”，系统按 30 个自然日边界判定短期/长期；境外补助标准由管理员配置，默认 0 元/天。
...
- 查询并复核当前员工已通过的出差审批，将选中实例写入 OA 的 `RelateField`。多选审批必须公司、预算、映射后的出差类别一致且日期相邻或重叠；申请补助时，其连续日期区间必须完整覆盖补助日期。
...
| 境外出差 | 0 元/天 | 按半天规则自动计算，标准由管理员配置且允许为 0 |
...
- 六类每日标准均由管理员配置，默认依次为 `100/100/150/50/100/0` 元。出差补助默认不申请；申请时，员工侧只显示“境内市外项目”，后端按 30 个自然日边界选择短期或长期标准。境内商务、市外项目和境外出差自动计算；境内同市项目、公司内部出差必须提交 `policyConfirmed=true` 和本次有效天数。
...
新提交使用 `snapshotVersion=5`，冻结附件用途、证明材料关联、铁路类型、出差来源类别和日期区间；上传记录仍严格为顺序 0 的 `GENERATED_PDF` 和顺序 1 的 `GENERATED_EXCEL`，OA 附件控件仅两项。汇总最多 500 页并受单对象容量限制；生成或上传失败使用持久检查点恢复，原件只有本地来源记录。版本 1–4 按各自原字段序列化和 SHA-256 恢复，不改变已冻结 OA 请求。

数据库迁移头为 `20260907_0016`，包含证明材料用途、住宿明细和出差来源类别字段。迁移不重写历史快照；升级前应一致性备份 SQLite 与 staging，再通过原启动命令执行迁移并重启加载新代码，不对运行中的业务数据库单独清理或改写。
...
前端只提供一个“费用明细”区域；每笔费用集中显示票据、对应行程单/付款凭证、警告及一组操作，上传/识别中和未分配材料也显示在同一区域。批量选择后先为全部文件建立稳定占位，再逐份上传和逐份识别，显示整批进度；完成后统一加入费用。单份上传失败可就地重试或移除，重试不会重放已成功文件；已上传但识别失败的材料可单独重新识别或删除。切换报销/部门或退出时不采用旧响应。前端从后端公开配置读取技术保护上限，默认可处理 200 条费用明细。员工可预览来源、编辑金额、重新识别、删除或仅保留材料；文件关系由持久 ID 维护，不再重复渲染完整附件卡片和完整费用表。页面展示结构化结果，原始 OCR 文本不入库。
...
- 六类补助标准、短期/长期 30 天边界和同市/内部出差有效天数确认责任人。
