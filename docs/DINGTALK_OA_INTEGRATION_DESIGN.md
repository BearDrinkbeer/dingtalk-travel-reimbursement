...
文档状态：与当前实现对齐，生产验收前基线；数据库迁移基线：`20260907_0016`；适用技术栈：Vue 3 + TypeScript、FastAPI、SQLite、钉钉企业内部应用。
...
服务器会用原查询窗口重新查询实例 ID，再读取详情，证明这张审批仍在当前员工可选列表中。服务器保存标题、业务编号、出差日期、来源模板、Schema 指纹、查询窗口和复核时间，并解析当前公司与预算。多张审批必须同时满足所属公司、预算代码和报销“出差类别”一致，且每张出差日期必须与报销行程或费用日期有重叠。页面对不匹配的候选禁选并解释原因，已选审批始终可以移除。
...
0013 给现有文件表增加 `attachment_kind`，按已有明确引用回填行程用途。0015 扩展用途约束以接受 `hotel_bill`，保留原文件记录、引用、触发器及已锁定 v1/v2/v3 快照字节与哈希。分类元数据复用 `ocr_result_json._materialClassification` 持久化，对外以独立 `materialClassification` 字段返回。删除、改角色或改用途复用 revision/锁定保护，并原子清理失效的来源、行程单、住宿明细和付款凭证引用；只改文件名不确认用途。
...
一份独立来源文件对应一张票据。新建/可编辑费用的 `sourceFileId` 非空时，后端将 `receiptCount` 规范为 `1`；无来源的手工汇总仍可调整张数，材料附件不计张数。v3/v4 快照拒绝来源票据张数不为 1；锁定历史输入、v1/v2 快照与 Excel 预览仍保留原张数。

多选批次先显示固定的全部文件占位，同时展示“已上传 X/N、已识别 Y/N”。上传和 OCR 各有一条顺序通道：A 上传完成即开始识别，同时上传 B；每条通道最多一个请求在途，重 OCR 仍只有一个进程准入槽。两条通道都结束后才统一发布费用候选、匹配行程、重算并自动保存，避免列表逐张重排和重复计算。明确用途的单独材料上传及已识别文件的重试沿用普通串行操作。

单份失败保留该行原因并继续后续文件，不自动重放请求。上传失败后只读取最新报销版本供下一次上传使用；有失败的批次在两条通道均停止后统一回读输入和文件，再发布可用结果。前端版本号只增不减，OCR 的迟到回包只能更新对应文件，不能用旧清单覆盖其他新上传文件。整批处理期间禁止修改明细、删除、清空和复核/提交；切换报销、部门或卸载组件会取消本批请求并忽略迟到结果。若发现报销输入已被外部修改，则停止本批并提示重新载入，不把旧费用发布到新上下文。行程单识别只更新文件证据，不产生费用行或重新累计票面金额。
...
服务器按快照顺序合并：每笔费用的来源发票，紧接该笔选择的住宿明细、行程单和付款凭证，最后加入其余材料。同一个文件 ID 被多次引用时只放入一次；不同上传文件即使字节相同也分别保留。
...
费用通过 `paymentProofFileIds` 引用同报销、`ACTIVE`、`ATTACHMENT_ONLY + payment_proof` 的文件，至少一份。后端在保存引用、完整复核、正式提交和 v3/v4 快照回读时分别验证归属/用途或付款要求，不信任客户端的“是否必需”标记；缺少必要凭证时返回 `409 / REIMBURSEMENT_DRAFT_NOT_READY`，不锁定或创建提交。
...
凡费用类别为 `lodging`，不论金额、来源 OCR 类型或手工录入方式，都必须通过 `hotelBillFileIds` 关联至少一份同报销、`ACTIVE`、`ATTACHMENT_ONLY + hotel_bill` 的材料。金额超过 500 元时还须另有付款凭证。服务端在保存时验证引用归属及用途，在复核、正式提交和 v4 快照中验证必需材料；缺失时保持草稿可编辑并提示补齐。
...
页面出差类型为“境内商务出差”“境内市外项目”“境内同市项目”“公司内部出差”和“境外出差”。境内类型保留现有半日天数、日补助和政策确认规则；出发/返回时段以中午 12:00 为上午、下午的分界。

境外使用 `tripType=overseas`，每日补助标准由管理员在系统设置中配置，默认 `0.00` 元/天，允许为 0，上限 `10000.00` 元/天。员工只选择出发、返回日期和上午时段，不手工填写补助总额。服务器按半天规则计算有效天数，再与当前境外每日标准相乘，生成最终人民币补助。

页面合计、自动保存、Excel 及 OA 明细共用服务器计算结果，Excel 和 OA 均显示“有效天数 × 每日标准”的计算信息。快照冻结有效天数、每日标准及总额并校验三者一致；外币票据人民币确认仍按 8.5 单独执行。
...
- `snapshotVersion=5`、出差模板公司/预算来源映射、逐笔来源出差类别及对应关系、日期区间映射、境外补助的有效天数/每日标准/总额和整个规范 JSON 的 SHA-256。

后台只使用这份快照，不再读取前端当前表单作为权威数据。数据库触发器禁止在提交创建后修改快照。

已存在的版本 `1/2/3/4` 快照仍按其原有字段和规则序列化，保持规范字节、SHA-256 与冻结请求不变；住宿明细、境外出差类型及公司/预算来源映射从 v4 起执行，逐笔来源类别和日期区间从 v5 起执行，不能伪装成更早版本后省略校验。付款凭证和来源张数规则从 v3 起执行；v1/v2 保持原有规则。后台恢复旧版本任务沿用对应版本的校验，不要求其补入新增来源字段。升级不会重写历史快照、改变已提交 OA 或把恢复任务转成一张新审批。

正式表单的 `DDDateRangeField` 同时映射开始、结束和时长三个逻辑项，向钉钉实际仅发送一个控件值：两个 `YYYY-MM-DD` 字符串的 JSON 数组。出差实例日期读取及报销回读使用同一严格格式，不能解析日期的实例不作为可选项，不猜测其他数组结构。来源“出差类型”按每张实例转换为报销选项：商务 → 境内商务，内部长期/短期 → 公司内部，境外长期/短期 → 境外商务，项目类别保持对应选项。多选仍要求转换后的类别、公司和预算一致。预算对应只忽略空白，歧义或名称不一致不按编号强配。

正式公司待确认请求体见 `config/production-oa-confirmation.json`，包含实际读取的字段与指纹，不自动导入数据库。关联范围未知时必须真实验证后确认；本地测试与模板只读成功不代表生产创建、附件上传或关联已经验收。
...
- 启动前执行 Alembic 到 `20260907_0016`；readiness 会检查当前 revision 和关键字段。0016 保存逐笔已核对的来源出差类别。升级先一致性备份 SQLite 和 staging，再通过原启动命令迁移/重启加载代码，保留全部业务记录与证据；0015 在存在 `hotel_bill` 文件或 v4 提交时拒绝降级，0013 对非 `other` 用途或 v3 以上提交继续保留降级保护；
...
| 境外每日补助可在设置中配置、默认 0 并按半天规则自动计算 | 已实现 | `test_overseas_subsidy.py`、设置 API、费用 store、补助卡片和快照测试 |
| 每笔住宿必附住宿明细文件、超过 500 元另附付款凭证 | 已实现 | 住宿明细文件解析、证明校验、文件引用、迁移和 v4 快照测试 |
...
| 出差审批 | 当前员工隔离、已完成且同意、120 天窗口、分页上限、来源字段精确解析/缺失/歧义、公司预算类别多选一致性、客户端覆盖拒绝、清空保留材料、旧草稿重新确认、保存和提交再复核 |
...
| 快照/payload | 规范 JSON、SHA-256、防篡改、10 字段、关联数组、预算完整标签、版本 1/2/3 原字节及 SHA 恢复、版本 2/3/4 两附件、v4 新语义拒绝写入旧版本、旧版本 worker 恢复 |
...
2026-09-07 统一材料上传、行程推荐和逐笔付款凭证交互最终冻结后，由独立验证角色重新执行：

- 后端 `pytest -o addopts='' -q`：998 通过，50.08 秒，11 条既有 SQLite datetime 弃用 warning；`ruff check app tests migrations` 通过。
- 前端完整测试：27 个文件、287 项通过；lint、typecheck、生产 build 通过。构建仍提示单个压缩 chunk 超过 500 kB，未作为构建失败处理。
- 新增回归覆盖已分类用途在重识别成功/失败/取消/版本冲突后保持稳定；同值用途确认不重跑 OCR、不覆盖人工字段；来源实际转换为证明时经过确认，保留原件并清理对应费用；数字文本缺字段时只补一次 OCR、合并互补文字、保留本地二维码警告及补读失败后的已有候选。
- 本地真实 10 份 PDF 的分类结果为 6 份费用来源（4 份打车、2 份铁路）和 4 份行程单。将其结构化结果实际输入当前前端匹配函数：严格自动匹配 3 对；未知交通类型的 9.20 元票据产生 1 条需人工确认的准确建议；确认后共 4 对，零错误关联。人工拒绝、已有链接及重复候选歧义另行验证，隔离样本匹配测试共 3 项通过。
- 最终 OCR 补丁后，重新通过当前 `OcrService` 和真实隔离 worker 只读处理 9.20 元原始 PDF，金额、日期、路线和匹配证据保持一致，交通类型仍未擅自变为网约车；此次本地处理约 0.326 秒。OpenCV 输出一条二维码 ECI 支持警告，结构化匹配字段未变化；该单份测量不代表端到端速度或一般票据识别准确率。
- 额外 2 项独立本地集成验证通过：AUTO 分类取消后由真实本系统 API 重试恢复；两条费用共享行程单/付款凭证时，提交快照仅收录每份文件一次，顺序保持费用来源、行程单、付款凭证。现有本地 worker 全链路回归仍验证最终 PDF 与 Excel 两个附件。

上述测试仅写隔离测试数据库和测试产物；没有修改正在使用的业务数据库、启动正式 OA worker 或创建新的远端审批。此前真实企业验收记录不因本轮本地回归扩大范围；Compose、Nginx 和真实钉钉客户端未在这一增量中重复验收。

2026-09-07 出差审批来源、境外补助和住宿明细增量验证：最终后端全量 `pytest -o addopts='' -q` 为 1146 通过，保留 11 条既有 SQLite datetime 弃用警告；前端 30 个文件、363 项测试通过，typecheck、lint、build、后端 Ruff 和 `git diff --check` 通过。build 保留既有主包超过 500 kB 提示。回归覆盖来源映射和多选一致性、旧可编辑草稿重新确认、境外总额/关闭恢复/Excel、任意金额的住宿费用必须提供住宿明细文件及独立付款要求、引用清理和两附件顺序；冻结 v3 原字节/哈希、v1–3 新语义拒绝、旧版本 worker 恢复、0015 升级证据与触发器保留/降级保护及 readiness 均通过。隔离 UI 验证了复用住宿明细文件后仅剩付款凭证待办、已补住宿明细文件区域的中性样式；最后纯样式调整另有 108 项专项及 lint 通过。测试未迁移正在使用的业务数据库，未操作真实 OA，隔离验收服务与页面已关闭。
...
- [ ] SQLite 已迁移到 `20260907_0016`，SQLite 与 staging 持久卷均可备份恢复；
