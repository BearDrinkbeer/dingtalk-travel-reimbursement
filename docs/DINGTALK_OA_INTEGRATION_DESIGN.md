...
文档状态：与当前实现对齐，生产验收前基线；数据库迁移基线：`20260909_0017`；适用技术栈：Vue 3 + TypeScript、FastAPI、SQLite、钉钉企业内部应用。
...
已存在的版本 `1/2/3/4/5` 快照仍按其原有字段和规则序列化，保持规范字节、SHA-256 与冻结请求不变；住宿明细、境外出差类型及公司/预算来源映射从 v4 起执行，逐笔来源类别和日期区间从 v5 起执行，逐审批补助项从 v6 起执行，不能伪装成更早版本后省略校验。付款凭证和来源张数规则从 v3 起执行；v1/v2 保持原有规则。后台恢复旧版本任务沿用对应版本的校验，不要求其补入新增来源字段。升级不会重写历史快照、改变已提交 OA 或把恢复任务转成一张新审批。
...
- Session Cookie 为 HttpOnly；HTTP 入口使用 `Secure=false`，只有所有入口均为 HTTPS 时才启用 `Secure`；CSRF token 只保存在前端内存并由 `/api/me` 轮换；
...
- 启动前执行 Alembic 到 `20260909_0017`；readiness 会检查当前 revision 和关键字段。0016 保存逐笔已核对的来源出差类别，0017 清理已取消的 OA 冒烟测试配置。升级先一致性备份 SQLite 和 staging，再通过原启动命令迁移/重启加载代码，保留全部业务记录与证据；0015 在存在 `hotel_bill` 文件或 v4 提交时拒绝降级，0013 对非 `other` 用途或 v3 以上提交继续保留降级保护；
...
- [ ] SQLite 已迁移到 `20260909_0017`，SQLite 与 staging 持久卷均可备份恢复；
