...
11. 出差补助为可选项，默认不申请。申请时以公司时区 `12:00` 为半天边界；五类政策标准由管理员配置，默认分别为 `100/100/150/50/100` 元。员工只选择“市外项目”，系统按 30 个自然日边界自动选择短期或长期标准并计算天数；同市和内部出差由用户显式确认本次有效天数。
...
- 商务出差和市外项目由后端自动计算；超过 30 个自然日自动按市外长期项目标准处理。同市和内部两类只接受用户显式确认的有效天数，且不能超过半天规则计算出的行程天数。
...
桌面 `差旅费报销单-王广硕.xlsx` 是当前确认的公司格式样例，但包含示例业务值和公式，不能直接提交。正式模板以其 `B:I` 八列表格、微软雅黑字体、灰色身份栏、蓝色总计栏、列宽、行高、边框、总金额人民币会计格式和页边距为基准，制作成单工作表脱敏版本。
...
- 商务出差和市外项目短期的开发默认标准均为 `100.00`；市外项目长期默认 `150.00`、同市项目默认 `50.00`、公司内部出差默认 `100.00`，五项后台政策标准都可由管理员调整。市外长期项目与短期项目一样按半天规则自动计算；同市项目和内部出差由用户确认本次有效天数（0.5 天单位），不能填写或覆盖每日标准；公司内部出差可明确选择无锡—苏州等制度规定的不补助例外，此时标准和金额按 `0.00` 计算。
...
flowchart LR
    D[钉钉客户端] --> V[Vue 3 H5]
    V -->|同源 HTTPS / HttpOnly Cookie| F[FastAPI 单体应用]
    F --> DT[钉钉 OpenAPI]
...
- 公司可信边缘层提供 HTTPS、证书、HSTS 和限流；容器内 Nginx 只提供同源 Vue 静态文件与 `/api` 反向代理，并丢弃调用方转发链。
...
日期使用 `YYYY-MM-DD`，时间使用本地 `HH:MM` 且不接受秒、时区偏移、空白或非补零格式。金额和有效天数在 JSON 中使用十进制字符串，后端进入领域层后转 `Decimal` 并统一量化。管理员配置的五类每日标准必须大于 `0` 且不超过 `10000.00` 元；员工不能在报销请求中提交每日标准。人工确认有效天数必须在 `0` 至 `366` 天之间、以 `0.5` 天为单位且不超过本次行程按半天规则得到的天数。补助明细是后端生成、`receiptCount=0` 的普通输出行，排序日期和发生日期取返回日期，金额始终由后端计算。同市和内部两类的确认值只存在当前页面和本次请求中。
...
- 生产属性：`HttpOnly; Secure; SameSite=Lax; Path=/`。
...
自动类型（包括长短期市外项目）不接收前端提交的补助天数、每日标准或补助总额；这些值统一从时间和系统设置重新计算。同市和内部两类必须提交 `policyConfirmed=true` 以及本次 `confirmedEffectiveDays` 十进制字符串，但仍不接收每日标准或补助总额。公司内部出差只有显式提交 `noSubsidyException=true` 时才按制度例外计算为 0 元。
...
- 自动类型按 12:00 半天边界计算；同市和内部两类只使用显式确认的半天单位有效天数，该值不能超过同一行程按半天规则得到的天数。
- 五类每日补助标准只从设置表读取；员工请求不能覆盖。短期连续天数必须 `<=30`，长期必须 `>30`；公司内部出差的不补助例外由显式布尔值选择。
...
- 记录制度截图仅作背景资料；V1 明确采用五类出差与 12:00 半天边界。管理员分别配置五类标准，默认 `100/100/150/50/100`；同市和内部两类要求本次显式确认有效天数，内部出差可明确选择不补助例外。公司正式确认仍是生产上线验收依赖。
...
- 项目搜索、选择、手工项目文本。
- 五类出差、开始/结束日期时间、12:00 半天规则和补助计算 API。
- 同市和内部出差显式确认本次有效天数，员工不能覆盖管理员配置的每日标准；内部出差可显式选择不补助例外。
- 集中的 17 类费用契约，以及手工明细新增、编辑、删除。
- 自动汇总票据金额、补助、总金额、人民币大写预览和票据张数。
- 项目/预算代码和五类每日补助标准管理页；后端管理员鉴权。
- PC 表格和移动卡片布局。
...
- 后端拒绝任何员工伪造的补助标准或总额；同市和内部两类要求显式确认有效天数并只计算本次金额。短期 `<=30` 个自然日、长期 `>30` 个自然日，二者都自动计算。
...
- 公司可信边缘层的 HTTPS/限流，以及应用 Nginx 的安全头、同源代理和合理超时；应用 Nginx 不信任外部 X-Forwarded-For 做限流。
- 区分 `/api/health` 存活检查与 `/api/ready` 依赖就绪检查；OCR 关闭不阻断手工闭环，启用后模型与锁定依赖是就绪门禁。
- 容器内 Nginx 作为同源应用边界，默认回环发布；公司外层 HTTPS 入口负责证书、HSTS 和可信域名，不在仓库生成证书。
...
- 五类管理员标准使用 `Decimal` 且前端不能覆盖；同市和内部两类必须显式确认有效天数，内部不补助例外必须显式选择。
- Decimal 量化、补助和所有合计。
- 人民币大写边界。
...
- Docker Compose 部署、HTTPS、健康检查和 Secret 管理就绪。
...
2. 确认五类每日补助标准、短期/长期 30 天边界、12:00 整点归属，以及同市/内部出差本次有效天数确认责任人和内部出差不补助例外。
3. 项目初始清单及管理员钉钉 userId。
4. 钉钉 CorpId、Client ID、Client Secret、应用权限和已备案 HTTPS 域名。
